...
public class OveritBowItem extends ProjectileWeaponItem implements Vanishable {

    public float dam = 3.5f;
    public static float chargetime = 10f;
    public OveritBowItem(Item.Properties properties) {
        super(properties);
    }
            ...
            if (!itemstack.isEmpty() || flag) {
                ...
                if (!((double)f < 0.1D)) {
                    ...
                    if (!p_40668_.isClientSide) {
                        ...
                        if (j > 0) {
                            abstractarrow.setBaseDamage(abstractarrow.getBaseDamage() + (double)j * 0.5D + dam);
                        }else{
                            abstractarrow.setBaseDamage(dam);
                        }
    ...
    public static float getPowerForTime(int p_40662_) {
        float f = (float)p_40662_ / chargetime;
        f = (f * f + f * 2.0F) / 3.0F;
        if (f > 1.0F) {
            f = 1.0F;
        }
